...
def macro_mirco_mean_stdv(data,ext_val=False):
    ...
    def micro(df):
        ...
        results = {
            'F1':               2*TP/(2*TP+FN+FP),
            'Cohen Kappa':      (2*(TN*TP)-(FN*FP))/(((TP+FP)*(FP+TN))+((TP+FN)*(FN+TN))),
            'CK2':              ((TP+TN)-(((FN+TP)*(TP+FP)+(FN+TN)*(FP+TN))/(TP+TN+FP+FN)))/(((TP+TN+FN+FP)-(((FN+TP)*(TP+FP)+(FN+TN)*(FP+TN))/(TP+TN+FP+FN)))),
...
def format_to_table(results):
    def splitting_method(string):
        if "_" in string:
    ...
    for grouping in results:
        working = results[grouping].copy()
        working['Grouping'] = grouping
        working['Kernel'] = working['model'].apply(lambda  x: splitting_method(x)[-1])
        working['model'] = working['model'].apply(lambda  x: splitting_method(x)[0])
        
        working = working.drop(['Cohen Kappa'],axis=1).rename(columns={'CK2':'Cohen Kappa'})
        working = working[['encoding','Grouping','Kernel','model','sensitivity','specificity','accuracy','AUROC','F1','Cohen Kappa']]
        if first:
            first = False
            output = working
        else:
